    ...
    if (depth == nb_towns) {
        ...
        for (i = 0; i < nb_towns; i++) {
            ...
            if (!present (town, depth, path)) {
                ...
                #pragma omp tasks
                if (current_length+dist >= min_distance)
...
void tsp () {
    #pragma omp parallel  default(none) shared(min_distance, nb_towns,d_matrix, dist_to_origin)
    {
        int *path = (int*) malloc(sizeof(int) * nb_towns);
        path[0] = 0;
        int town, dist;
        #pragma omp for schedule(auto 2)
